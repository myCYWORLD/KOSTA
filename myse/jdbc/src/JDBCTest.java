	...
	public static void search(){
		...
		try {
			...
			while(rs.next()) { //커서를 다음 행(row)으로 이동 시켜주는 메서드
				int id = rs.getInt("employee_id"); //= rs.getint(1)도가능 java인덱스의 0번
				// sql의 number타입 가지고 오려면 java의 int로 가지고 와야함

				String name = rs.getString("first_name"); //= rs.getint(1) java인덱스의 1번
				// sql의 varchar타입 가지고 오려면 java에서는 String ->get.char가 없음
		...
		//7. DB 연결해제 -> 연결 해줬으면 꼭 해제를 해줘야함 (지저분해져도 안전하게 클로즈하는게 나음)
		if(rs != null) {
			try {
				rs.close();
			} catch (SQLException e) {
