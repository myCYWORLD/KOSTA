...
public class BoardOracleRepository implements BoardRepository {
	...
	public Board selectByBoardNo(int boardNo) throws FindException {
		...
		try {
			...
			return session.selectOne("com.my.mapper.BoardMapper.selectByBoardNo", map);
		}catch(Exception e) {
			e.printStackTrace();
			throw new FindException(e.getMessage());
		}finally {
	...
	public void delete(int boardNo) throws RemoveException {
		...
		try {
			session = sqlSessionFactory.openSession();
			deleteReply(session, boardNo); //답글을 삭제하는 작업과 게시글 삭제하는 작업이 같은 세션을 써야함
			deleteBoard(session, boardNo); //문제가 생기면 롤백을 해야하기 때문
		}catch(Exception e) {
			//트랜잭션 롤백
			e.printStackTrace();
